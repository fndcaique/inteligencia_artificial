...
kMeans1 <- function(dados, k = 2) {
  ...
  #atribui??o aleatoria dos rotulos
  for (i in 1:nrow(dados)) {
    rownames(dados)[i] <-
      sample(rotulo, 1) # sample embaralha o conjunto de dados e o ,1 retorna uma lista com a quantidade de valores qu esta na lista
  }
  
  centroids <-
    colMeans(dados[rownames(dados) == 1,]) # retorna a m?dia por coluna
