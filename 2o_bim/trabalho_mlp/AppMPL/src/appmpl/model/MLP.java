...
public class MLP implements Sujeito{
    ...
    public ArrayList backpropagation(Arquitetura arquitetura, double[][] dados, int[] treino, double taxa, double limiar, ArrayList<String> rotulo_classes, int max_epocas) {
        arq = arquitetura;
        int epocas = 0, i, j, k;
        Object saida;
        ...
        while (erro_quadratico > limiar && epocas < max_epocas) {
            ...
            int[] random = new int[treino.length];
            Map<Integer,Integer> m = new HashMap<Integer, Integer>();
            int u,x=0;
            boolean flag;
            do{
                u = (int)(Math.random()*random.length);
                if(null == m.get(u))
                {
                    m.put(u, 1);
                    random[x++] = u;
            ...
            for (i = 0; i < treino.length; i++) {
                ...
                for (j = 0; j < entrada.length; j++)
                    entrada[j][0] = dados[treino[random[i]]][j];
                saida = rotulo_classes.get((int) dados[treino[random[i]]][j]);
